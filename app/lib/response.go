...
var validateMessages = map[string]string{
	...
	"emptyString": "%v value is empty %v",
...
// ErrorHTTP send http error custom
func ErrorHTTP(c *fiber.Ctx, code int, message ...string) error {
	if len(message) == 0 {
		m := "error"
		if r, ok := HTTPResponse[code]; ok {
			m = r
		}
		message = append(message, m)
	}

	return Send(c, code, message[0])
}
...
// HTTPResponse var collect response http error and its message
var HTTPResponse map[int]string = map[int]string{
	400: "Bad Request",
	401: "Unauthorized",
